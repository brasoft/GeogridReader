 ...
 *
 * Created on 23. Februar 2005, 01:09
 */
 ...
 */
    ...
    private static double TranMerc_a = 6378137.0;              /* Semi-major axis of ellipsoid i meters */
    private static double TranMerc_f = 1 / 298.257223563;      /* Flattening of ellipsoid  */
    private static double TranMerc_es = 0.0066943799901413800; /* Eccentricity (0.08181919084262188000) squared */
    private static double TranMerc_ebs = 0.0067394967565869;   /* Second Eccentricity squared */
    
    /* Transverse_Mercator projection Parameters */
    private static double TranMerc_Origin_Lat = 0.0;           /* Latitude of origin in radians */
    private static double TranMerc_Origin_Long = 0.0;          /* Longitude of origin in radians */
    private static double TranMerc_False_Northing = 0.0;       /* False northing in meters */
    private static double TranMerc_False_Easting = 0.0;        /* False easting in meters */
    private static double TranMerc_Scale_Factor = 1.0;         /* Scale factor  */
    
    /* Isometeric to geodetic latitude parameters, default to WGS 84 */
    private static double TranMerc_ap = 6367449.1458008;
    private static double TranMerc_bp = 16038.508696861;
    private static double TranMerc_cp = 16.832613334334;
    private static double TranMerc_dp = 0.021984404273757;
    private static double TranMerc_ep = 3.1148371319283e-005;
    
    /* Maximum variance for easting and northing values for WGS 84. */
    private static double TranMerc_Delta_Easting = 40000000.0;
    private static double TranMerc_Delta_Northing = 40000000.0;
    
/* These state variables are for optimization purposes. The only function
 * that should modify them is Set_Tranverse_Mercator_Parameters.         */
    
    // Konstanten
    //private static double PI_OVER =        (Math.PI/2.0e0);            /* PI over 2 */
    private static double MAX_LAT =         ((Math.PI * 89.99)/180.0);    /* 90 degrees in radians */
    private static double MAX_DELTA_LONG =  ((Math.PI * 90)/180.0);    /* 90 degrees in radians */
    private static double MIN_SCALE_FACTOR=  0.3;
    private static double MAX_SCALE_FACTOR=  3.0;
    
    
    private static int TRANMERC_NO_ERROR =          0x0000;
    private static int TRANMERC_LAT_ERROR=          0x0001;
    private static int TRANMERC_LON_ERROR =         0x0002;
    private static int TRANMERC_EASTING_ERROR =     0x0004;
    private static int TRANMERC_NORTHING_ERROR =    0x0008;
    private static int TRANMERC_ORIGIN_LAT_ERROR=   0x0010;
    private static int TRANMERC_CENT_MER_ERROR =    0x0020;
    private static int TRANMERC_A_ERROR  =          0x0040;
    private static int TRANMERC_INV_F_ERROR =       0x0080;
    private static int TRANMERC_SCALE_FACTOR_ERROR= 0x0100;
    private static int TRANMERC_LON_WARNING   =     0x0200;
 ...
 */
    ...
    private static double SPHSN(double Latitude) {
        return (TranMerc_a / Math.sqrt( 1.e0 - TranMerc_es *
                Math.pow(Math.sin(Latitude), 2)));
    }
    
    private static double SPHSR(double Latitude) {
        return (TranMerc_a * (1.e0 - TranMerc_es) /
    ...
    static int Set_Parameters(double a,
            double f,
   ...
   */
        ...
        if ((Central_Meridian < -Math.PI) || (Central_Meridian > (2*Math.PI))) { /* origin longitude out of range */
        ...
        if (Error_Code==0) { /* no errors */
            ...
            TransMercPoint DeltaEastNorth = new TransMercPoint();
            Convert_From_Geodetic(MAX_LAT,
                    MAX_DELTA_LONG,
                    DeltaEastNorth);
            TranMerc_Delta_Easting = DeltaEastNorth.Easting;
            TranMerc_Delta_Northing = DeltaEastNorth.Northing;
            Convert_From_Geodetic(0,
                    MAX_DELTA_LONG,
                    DeltaEastNorth);
            TranMerc_Delta_Easting = DeltaEastNorth.Easting;
            //dummy_northing = DeltaEastNorth.Northing;
   ...
   */
    ...
    *Scale_Factor = TranMerc_Scale_Factor;
    ...
    static int Convert_From_Geodetic(double Latitude,
            double Longitude,
            TransMercPoint out)
   ...
   */
     ...
     */
            ...
            if (Math.abs(dlam) > (9.0 * Math.PI / 180)) { /* Distortion will result if Longitude is more than 9 degrees from the Central Meridian */
            ...
            out.Northing = TranMerc_False_Northing + t1 + Math.pow(dlam,2.e0) * t2
            ...
            out.Easting = TranMerc_False_Easting + dlam * t6 + Math.pow(dlam,3.e0) * t7
    ...
    static int Convert_To_Geodetic(
            double Easting,
            double Northing,
            GeodeticPoint out) {      /* BEGIN Convert_Transverse_Mercator_To_Geodetic */
   ...
   */
        ...
        if (Error_Code==0) {
            ...
            * Math.pow(TranMerc_Scale_Factor,4));
            ...
            out.GeoLat = ftphi - Math.pow(de,2) * t10 + Math.pow(de,4) * t11 - Math.pow(de,6) * t12
                    + Math.pow(de,8) * t13;
            ...
            /* Longitude */
            out.GeoLon = TranMerc_Origin_Long + dlam;
            while (out.GeoLat > (90.0 * Math.PI / 180.0)) {
                out.GeoLat = Math.PI - out.GeoLat;
                out.GeoLon += Math.PI;
            ...
            while (out.GeoLat < (-90.0 * Math.PI / 180.0)) {
                out.GeoLat = - (out.GeoLat + Math.PI);
                out.GeoLon += Math.PI;
                if (out.GeoLon > Math.PI)
                    out.GeoLon -= (2 * Math.PI);
            }
            if (out.GeoLon > (2*Math.PI))
                out.GeoLon -= (2 * Math.PI);
            if (out.GeoLon < -Math.PI)
                out.GeoLon += (2 * Math.PI);
